...
resource "aws_security_group_rule" "rjs_ingress_rules" {
  type              = "ingress"
  from_port         = 3000
  to_port           = 3000
  protocol          = "tcp"
  cidr_blocks       = ["0.0.0.0/0"]
  security_group_id = aws_security_group.rjs_app_sg.id
}

resource "aws_security_group_rule" "njs_ingress_rules" {
  type              = "ingress"
  from_port         = 5000
  to_port           = 5000
  protocol          = "tcp"
...
resource "aws_security_group_rule" "rjs_egress_rules" {
  type              = "egress"
  to_port           = 0
  protocol          = "-1"
  from_port         = 0
